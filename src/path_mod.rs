...
pub trait SuperPathBuf {
    ...
    // fn write_text__(self, text: &str) -> Self;
    fn create(&self) -> &Self;
    // fn create_(&mut self) -> &mut Self;
    // fn create__(self) -> Self;
    fn delete(&self) -> &Self;
    // fn delete_(&mut self) -> &mut Self;
    fn home() -> Self; 
    fn cwd() -> Self; 
    fn tmp() -> Self; 
...
impl SuperPathBuf for PathBuf {
    ...
    // fn write_text__(self, text: &str) -> Self {self.parent().unwrap().to_path_buf().create(); let mut file = File::create(&self).expect("Failed to create file"); file.write_all(text.as_bytes()).expect("Failed to write file"); return self}
    ...
    fn delete(&self) -> &Self {return self}
    // fn delete_(&mut self) -> &mut Self {return self}
...
// * if an immutable reference is consumed by method, you can only return an immutable reference. Another downside is you can't create an object and use a method of reference in a chained one-liner.
